        ...
        <aside className="flex flex-col bg-[#fcfaf3] shadow-md rounded-lg p-4 justify-center">
          {announcements.map((announcement, index) => (
            <article key={index} className="p-4">
              <a href={announcement.link} className="text-lg font-bold text-[#007773] hover:underline flex pb-4">
                {announcement.title}
              </a>
              <p className="text-sm text-gray-600">{announcement.description}</p>
              <a href={announcement.link} className="text-xs text-[#007773] hover:underline flex pt-4">
                ادامه خبر
              </a>
            </article>
          ))}
